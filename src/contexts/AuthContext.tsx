...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  useEffect(() => {
    ...
    const unsubscribePlaySignUps = onSnapshot(qPlaySignUps, (querySnapshot) => {
      ...
      toast({ 
        variant: "destructive", 
        title: `Erro ao Buscar Inscrições do Aula`,
        description: `Não foi possível carregar os dados das inscrições do Aula. Verifique suas Regras de Segurança e Índices do Firestore. Erro: ${getFirebaseErrorMessage(error.code, "Falha ao carregar inscrições Aula.")}`,
        duration: 10000
      });
    });
